---
title: "Publications & Patents"
---

My research focuses on metasurface-enabled optical computing, structured light manipulation, nonlocal optical analog processing, and integrated multidimensional sensing. Selected representative works are highlighted below.

## Selected Publications

### Simultaneous Sorting of Arbitrary Vector Structured Beams with Spin-Multiplexed Diffractive Metasurfaces

**Xiaoxin Li**, Rui Feng, Fangkui Sun, Yanxia Zhang, Qi Jia, Donghua Tang, Bojian Shi, Hang Li, Yanyu Gao, Wenya Gao, Yongyin Cao, Weiqiang Ding

**Advanced Photonics Nexus (2024)**  
**DOI:** 10.1117/1.APN.3.3.036010

**Summary:**  
A compact spin-multiplexed diffractive metasurface framework for simultaneous sorting and detection of arbitrary vector structured beams.

**Highlights:**  
- Supports simultaneous identification of vector beams based on LG, HG, and BG modes  
- Uses cascaded diffractive metasurfaces as a compact optical neural network  
- Provides a practical route toward integrated and high-dimensional optical communication systems  

---

### Diffractive Metasurface-Enabled Single-Shot Characterization of Hybrid-Order Poincaré Beams

**Xiaoxin Li**, Bojian Shi, Qi Jia, Yanxia Zhang, Yanyu Gao, Wenya Gao, Donghua Tang, Yongyin Cao, Fangkui Sun, Rui Feng, Weiqiang Ding

**Advanced Optical Materials (2025)**  
**DOI:** 10.1002/adom.202403405

**Summary:**  
A single-shot and real-time characterization strategy for hybrid-order Poincaré beams using cascaded diffractive metasurfaces and electronic neural networks.

**Highlights:**  
- Enables direct single-shot characterization without conventional multi-step polarization projection  
- Diffractive metasurfaces function as an optical processor for vector-mode transformation  
- Achieves low average reconstruction errors for the key Poincaré parameters  

---

### Polarization-Independent, High-NA Edge Detection with Nonlocal Metasurfaces

Yalong Liu, **Xiaoxin Li**, Bojian Shi, Qi Jia, Yanxia Zhang, Yanyu Gao, Wenya Gao, Fangkui Sun, Yongyin Cao, Rui Feng, Weiqiang Ding

**Optics Letters (2025)**  
**DOI:** 10.1364/OL.562895

**Summary:**  
A nonlocal dielectric metasurface for polarization-independent optical edge detection with both high numerical aperture and broadband response.

**Highlights:**  
- Polarization-independent operation under arbitrary polarization conditions, including unpolarized illumination  
- Broadband edge detection with a 48 nm operating range centered around 1064 nm  
- High numerical aperture up to 0.5 for practical optical computing and imaging applications  

---

### Bilayer Metasurface-Enabled High-Sensitivity Near-Infrared Computational Spectrometer

Juntong Chen, **Xiaoxin Li**, Bojian Shi, Qi Jia, Yanxia Zhang, Wenya Gao, Yanyu Gao, Wentong Shi, Yongyin Cao, Hongyan Shi, Fangkui Sun, Rui Feng, Weiqiang Ding

**Optics Communications (2025)**  
**DOI:** 10.1016/j.optcom.2025.132051

**Summary:**  
A bilayer metasurface computational spectrometer that combines spectral encoding and broadband achromatic focusing for high-sensitivity near-infrared sensing.

**Highlights:**  
- Ultra-compact footprint of only 21 × 21 μm  
- Sensitivity improved by about 60% compared with conventional structures  
- Spectral reconstruction fidelity reaches 93.5%  

---

### Efficient Coupling of Topological Photonic Crystal Waveguides Based on Transverse Spin Matching Mechanism

Bojian Shi, Qi Jia, **Xiaoxin Li**, Yanxia Zhang, Hang Li, Yanyu Gao, Wenya Gao, Xiaoyu Li, Donghua Tang, Tongtong Zhu, Shan Gao, Jing Yang, Rui Feng, Fangkui Sun, Chunying Guan, Yongyin Cao, Cheng-Wei Qiu, Weiqiang Ding

**Nature Communications (2025)**  
**DOI:** 10.1038/s41467-025-59941-6

**Summary:**  
A transverse-spin-matching mechanism for efficient coupling between topological photonic crystal waveguides and conventional strip waveguides.

**Highlights:**  
- Reveals transverse spin matching as the key coupling mechanism for TWG-related interconnections  
- Demonstrates theoretical and experimental transmission efficiencies of 96.3% and 94.2%, respectively  
- Provides a route toward compact and high-efficiency topological photonic chips  


## Patents

### Broadband Full-Stokes Polarization Imaging Device Based on Non-Orthogonal Four-Channel Polarization Splitting

**Status:** Patent application accepted  
**Application No.:** 202511764621.1  
**Application Date:** 2025-11-27  
**Applicant:** Harbin Institute of Technology  
**Inventors:** Ding Weiqiang, Li Xiaoxin, Feng Rui, Cao Yongyin, Sun Fangkui

A broadband full-Stokes polarization imaging device based on non-orthogonal four-channel polarization splitting.

---

### Polarization-State Characterization Method for Hybrid-Order Poincaré Beams Based on Diffractive Metasurfaces

**Status:** Patent application accepted  
**Application No.:** 202510183871.X  
**Application Date:** 2025-02-19  
**Applicant:** Harbin Institute of Technology  
**Inventors:** Li Xiaoxin, Feng Rui, Shi Bojian, Ding Weiqiang, Sun Fangkui

A diffractive-metasurface-based method for polarization-state characterization of hybrid-order Poincaré beams.

---

### Pixel-Level High-Efficiency Visible Bayer Color Filter Based on Metalens / Metasurface

**Status:** Patent application accepted  
**Application No.:** 202211486566.0  
**Application Date:** 2022-11-24  
**Applicant:** Harbin Institute of Technology

A pixel-level high-efficiency visible Bayer color filter based on a metalens / metasurface architecture.

---
